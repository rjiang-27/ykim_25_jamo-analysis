...
for (i in seq_along(unicode_list)) {
  ...
  unicode_proportions[[i]] <- data.frame(
    unicode = u,
    count = sum(str_detect(unique[["Jamo_Cleaned_Unicode"]], fixed(u))),
...
unicode_counts <- ggplot(unicode_df, aes(x = reorder(unicode, count), y = count)) +
  geom_col() +
  geom_text(
    aes(label = count),
    hjust = -0.1,
  ...
  labs(
    y = "Total Counts",
    x = "Jamo Unicode",
    title = "Counts of Each Jamo Unicode in the Dataset"
  )

ggsave("unicode_counts.pdf", plot = unicode_counts, width = 8, height = 6)
