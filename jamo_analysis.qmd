...
for (i in seq_along(ipa_unicode_list)) {
  ...
  ipa_unicode_proportions[[i]] <- data.frame(
    unicode = u,
    count = sum(str_detect(unique[["IPA_Syllable_Unicode"]], fixed(u))),
...
ipa_unicode_counts <- ggplot(ipa_unicode_df, aes(x = reorder(unicode, count), y = count)) +
  geom_col() +
  geom_text(
    aes(label = count),
    hjust = -0.1,
  ...
  labs(
    y = "Total Counts",
    x = "IPA Unicode",
    title = "Counts of Each IPA Unicode in the Dataset"
  )

ggsave("ipa_unicode_counts.pdf", plot = ipa_unicode_counts, width = 8, height = 6)
