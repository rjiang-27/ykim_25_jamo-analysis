...
for (i in seq_along(rules)) {
  ...
  # Logical vector: TRUE = row contains rule = 1 at least once
  contains_rule <- map_lgl(unique$Jamo_Rule_Matrix, ~ any(.x[, r] == 1))
  
  # Compute proportion removed
  proportion_removed <- mean(contains_rule)
  
  # Save as small data frame
...
ggplot(rule_proportion_df, aes(x = reorder(rule, prop_removed), y = prop_removed)) +
  geom_col() +
  coord_flip() +
  theme_minimal() +
  labs(
